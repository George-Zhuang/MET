# -----------------------------------------------------------------------------
# Multimoadl Embodied Tracking (MET)
...
class AgentSDG:
    ...
    def generate_data(self, config_file):
        ...
        if "robot" in self.config_dict:
            ...
            # commands_list = [
            #     "Nova_Carter Track Character_01",
            #     "Nova_Carter_01 Track Character_02",
            # ]
            commands_list = TrackManager.get_instance().generate_tracking_command()
            self._sim_manager.save_robot_commands(commands_list)  # Write commands to file
            self._sim_app.update()

        # Run data generation
        self._data_generator._init_recorder()
        skip_frames = self._settings.get("/persistent/exts/metspace/skip_starting_frames")
