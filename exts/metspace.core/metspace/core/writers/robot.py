...
class TaoRobotWriter(Writer):
        ...
        if self.video:
            ...
            try:
                for folder in rgb_folders:
                    Popen(
                        "ffmpeg -r 30 -f image2 -s 1920x1080 -start_number 0 -y -i {}/%d.jpeg -vcodec libx264 -crf 23 -pix_fmt yuv420p {}/video.mp4".format(
                            folder, folder
                        ),
                        shell=True,
                    )
    ...
    def write_robot_info(self):
        stage = omni.usd.get_context().get_stage()
        robot_root_prim = stage.GetPrimAtPath("/World/Robots")
        if not robot_root_prim.IsValid() or not robot_root_prim.IsActive():
            return []
        children = robot_root_prim.GetAllChildren()
        write_frame = {}
        write_frame["timestamp"] = ""
        write_frame["place"] = "city=Santa Clara/building=NVIDIA Voyager/room=Visitor Lobby"
        write_frame["objectCounts"] = [{"type": "AMR", "count": len(children)}]
        write_frame["frame_id"] = self._write_id
        location_objects = []

        for c in children:
            pos = omni.usd.get_world_transform_matrix(c).ExtractTranslation()
            new_robot = {}
            new_robot["id"] = str(c.GetPrimPath())
            new_robot["type"] = "AMR"
            new_robot["locations"] = [pos[0], pos[1]]
            location_objects.append(new_robot)
        write_frame["locationsOfObjects"] = location_objects
        buf = io.BytesIO()
        buf.write(json.dumps(write_frame).encode())
        file_path = os.path.join("amr_locations", f"{self._write_id}.json")
        self._backend.write_blob(file_path, buf.getvalue())


WriterRegistry.register(TaoRobotWriter)
